#!/usr/bin/env python
"""Phase 6-7 — + L_SC_corr + L_SC_mag (+edge)  (pipeline §21).
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default=str(ROOT / "outputs" / "checkpoints" / "sc"))
    a = ap.parse_args()
    cfg = TrainConfig(sc_mode=a.sc_mode, n_gen_per_pair=a.n_gen, max_pairs_per_step=a.max_pairs,
                      chunk=a.chunk, lr=a.lr)
    ck = run("sc", a.subjects, a.max_steps, Path(a.out), cfg, init_bundle=a.init_bundle,
