...
for i in $(seq 0 7); do
  python scripts/69_tissue_extract.py --start $i --stride 8 --source syn \
      > outputs/logs/tissue_syn_$i.log 2>&1 &
done
wait
grep -h "TISSUE DONE" outputs/logs/tissue_syn_*.log
