#!/usr/bin/env python
"""Phase 2 — ROI-pair ATM baseline (L_ATM 만)  (pipeline §21).
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default=str(ROOT / "outputs" / "checkpoints" / "roi_atm"))
    a = ap.parse_args()
    cfg = TrainConfig(sc_mode=a.sc_mode, n_gen_per_pair=a.n_gen, max_pairs_per_step=a.max_pairs,
                      chunk=a.chunk, lr=a.lr)
    ck = run("roi_atm", a.subjects, a.max_steps, Path(a.out), cfg, init_bundle=a.init_bundle,
